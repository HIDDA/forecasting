...
par(mar=c(0,5,1,1))
plot(hhh4owa, ylim = c(0,1.5*max(CHILI[OWA+1])),
     fan.args = list(fan.col = colorRampPalette(c("darkgreen", "gray90"))),
     observed.args = list(type = "p", pch = 20), key.args = list(),
     xaxt = "n", xlab = "")
## add scores
par(mar=c(5,5,0,1))
...
```{r sarimaxowa_scores}
sarimaxowa_scores <- dss_lnorm(x = CHILI[OWA+1],
                               meanlog = sarimaxowa[,"pred"],
                               sdlog = sarimaxowa[,"se"])
summary(unclass(sarimaxowa_scores))
```

```{r sarimaxowa_plot}
osaplot(
    quantiles = sapply(1:99/100, qlnorm, meanlog=sarimaxowa[,"pred"], sarimaxowa[,"se"]),
    probs = 1:99/100, observed = CHILI[OWA+1],
    scores = cbind(DSS = sarimaxowa_scores),
    start = OWA[1]+1, xlab = "Week", ylim = c(0,1.5*max(CHILI[OWA+1]))
)
```
