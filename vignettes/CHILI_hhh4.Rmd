...
```{r}
## use the first forecast period for a more detailed analysis
TEST1 <- TEST[[1]]
```

```{r hhh4sim}
fit1 <- update(hhh4fit, subset.upper = TEST1[1]-1)
hhh4sim <- simulate(fit1, nsim = 1000, seed = 1, subset = TEST1,
                    y.start = observed(CHILI.sts)[TEST1[1]-1,])
```

```{r hhh4sim_pit, fig.width = 3, fig.height = 3, echo = -1}
par(mar = c(5,5,1,1))
pit(
    x = CHILI[TEST1],
    pdistr = apply(hhh4sim, 1, ecdf)
)
```

```{r hhh4sim_scores}
hhh4sim_scores <- scores(hhh4sim, which = c("dss", "logs"))
summary(hhh4sim_scores)
```

```{r hhh4sim_plot, echo = -2}
hhh4sim_quantiles <- t(apply(hhh4sim, 1, quantile, probs = 1:99/100))
par(mar = c(5,5,1,1))
osaplot(
    quantiles = hhh4sim_quantiles, probs = 1:99/100,
    observed = CHILI[TEST1], scores = hhh4sim_scores[,"dss",drop=FALSE],
...
We could also use the plot method provided by **surveillance**:

```{r hhh4sim_plots, R.options = list(scipen=1)}
par(mfrow=c(1,2))
plot(hhh4sim, "fan", ylim = c(0,60000), xlab = "", ylab = "", epochsAsDate = TRUE,
     xaxis.tickFreq = list("%m"=atChange, "%Y"=atChange),
     xaxis.labelFreq = list("%Y"=atMedian), xaxis.labelFormat = "%Y")
plot(hhh4sim, "size", horizontal = FALSE, main = "size of the epidemic",
...
```{r hhh4for3}
doit <- function (testperiod) {
    t0 <- testperiod[1] - 1
    fit0 <- update(hhh4fit, subset.upper = t0)
    hhh4sim <- simulate(fit0, nsim = 1000, seed = 1, subset = testperiod,
                        y.start = observed(CHILI.sts)[t0,])
    list(t0 = t0,
         observed = as.vector(CHILI)[testperiod],
         scores = scores(hhh4sim, which = c("dss", "logs")),
         quantiles = t(apply(hhh4sim, 1, quantile, probs = 1:99/100)))
}
hhh4for3 <- lapply(TEST, doit)
```

```{r hhh4for3_plot, echo = -1, fig.show = "hold"}
par(mar = c(5,5,1,1))
t(sapply(hhh4for3, function (x) {
    osaplot(quantiles = x$quantiles, probs = 1:99/100,
            observed = x$observed, scores = x$scores[,"dss",drop=FALSE],
