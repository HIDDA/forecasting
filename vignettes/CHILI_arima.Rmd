...
```{r}
fit0 <- update(sarimaxfit, subset = TRAIN1)
sarimaxfor <- predict(fit0, n.ahead = length(TEST1), newxreg = sarimax_cov[TEST1,,drop=FALSE])
```
```{r, include = FALSE}
...
fc <- forecast(fit0, xreg = sarimax_cov[TEST1,,drop=FALSE], fan = FALSE)
