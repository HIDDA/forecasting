---
title: "Forecasting Swiss ILI surveillance counts using `auto.arima`"
...
output:
    rmarkdown::html_vignette:
        ...
        toc: TRUE
vignette: >
  %\VignetteIndexEntry{Forecasting Swiss ILI surveillance counts using 'auto.arima'}
...
arimafit <- auto.arima(CHILI[TRAIN1], lambda = 0, stepwise = FALSE, approximation = FALSE)
summary(arimafit)
```
...
sarimafit <- auto.arima(CHILI[TRAIN1], lambda = 0, stepwise = FALSE, approximation = FALSE,
    xreg = t(sapply(2*pi*TRAIN1/52.1775, function (x) c(sin=sin(x), cos=cos(x)))))
summary(sarimafit)
```
...
sarimaxfit <- auto.arima(CHILI[TRAIN1], lambda = 0, stepwise = FALSE, approximation = FALSE,
                         xreg = sarimax_cov[TRAIN1,])
summary(sarimaxfit)
```
...
CHILIdat[TRAIN1,"sarimaxfitted"] <- fitted(sarimaxfit)
CHILIdat[TRAIN1,c("sarimaxlower","sarimaxupper")] <-
...
sarimaxowa <- t(simplify2array(plapply(X = OWA, FUN = function (t) {
    ...
    if (t == max(TRAIN1)) # check that we are refitting the same model
...
```{r}
plot(forecast(arimafit, h=length(TEST1), fan=TRUE))
##predict(arimafit, n.ahead=length(TEST1))
```
