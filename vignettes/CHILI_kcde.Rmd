---
title: "Forecasting Swiss ILI surveillance counts using `kcde::kcde`"
...
output:
    rmarkdown::html_vignette:
        ...
        toc: TRUE
vignette: >
  %\VignetteIndexEntry{Forecasting Swiss ILI surveillance counts using 'kcde::kcde'}
...
Note: **kcde** will itself log-transform the CHILI counts.
...
```{r kcdefit, eval = !file.exists("kcdefit.RData"), results = "hide"}
kcde_control <- create_kcde_control(
    ...
    kernel_components = kernel_components,  # the complicated part (not shown)
    filter_control = NULL,
...
The estimation function `kcde()` just returns a list without a dedicated class.
It is unclear how to summarize the model fit or extract fitted values ...
...
```{r kcdeowa, eval = !file.exists("kcdeowa.RData"), results = "hide"}
library("pdtmvn")   # FIXME: do we need to attach that package?

## row indices in data corresponding to times at which we want to make a prediction
prediction_time_inds <- OWA + 1

## Allocate data frame to store results
num_rows <- length(prediction_time_inds)
kcdeowa <- data.frame(
    prediction_time = rep(as.Date(NA), num_rows),
...
for(prediction_time_ind in prediction_time_inds) {
    ## Set values describing case in kcdeowa
    kcdeowa$prediction_time[results_row_ind] <- CHILIdat$Index[prediction_time_ind]
    ...
    observed_prediction_target <-
        CHILIdat[prediction_time_ind, prediction_target_var, drop = FALSE]
    colnames(observed_prediction_target) <-
        paste0(prediction_target_var, "_horizon", prediction_horizon)
    kcdeowa$log_score[results_row_ind] <-
        kcde_predict(
            kcde_fit = kcdefit,
            prediction_data =
                CHILIdat[seq_len(analysis_time_ind), , drop = FALSE],
    ...
    kcdeowa$AE[results_row_ind] <-
        abs(kcdeowa$pt_pred[results_row_ind] -
            observed_prediction_target)
...
save(kcdeowa, file = "kcdeowa.RData")
```
```{r, include = FALSE}
load("kcdeowa.RData")
```
