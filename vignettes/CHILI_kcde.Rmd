...
We used the diagonal bandwidth parametrization.
                             ...
                             nsamples = 10000)
{
    ...
    scores$pt_pred <- apply(samples, 1, median) - 0.5
    ...
    return(list(scores = scores, samples = samples))
}
...
```{r kcdeowa_pit, echo = -1}
par(mar = c(5,5,1,1), las = 1)
surveillance::pit(x = kcdeowa$scores$observed, pdistr = apply(kcdeowa$samples, 1, ecdf))
```

```{r kcdeowa_scores}
summary(kcdeowa$scores[c("log_score", "AE")])
```

```{r kcdeowa_plot, echo = -1}
par(mar = c(5,5,1,1))
osaplot(
    quantiles = t(apply(kcdeowa$samples, 1, quantile, probs=1:99/100)), probs = 1:99/100,
    observed = kcdeowa$scores$observed, scores = kcdeowa$scores$log_score,
