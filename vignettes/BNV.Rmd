...
autoplot(BNV, population = 100000/popBE) +
    ...
    geom_vline(aes(xintercept = as.numeric(date)[max(TRAIN)] + .5), linetype = 2) +
...
```{r}
summary(mg_SCrecipower, maxEV = TRUE, reparamPsi = TRUE,
        amplitudeShift = TRUE, idx2Exp = TRUE)
```

```{r fitted_components}
## plot estimated endemic-epidemic decomposition
##plot(mg_SCrecipower, units = NULL, pch = 20,
...
plotHHH4_fitted_groups(mg_SCrecipower, groups = GROUPS, units = NULL, pch = 20,
...
plotHHH4_season_groups(mg_SCrecipower, component = "end", seasonStart = 27,
...
with(data.frame(time = epochInYear(BNV) + (year(BNV)-2011)*52,
                maxEV = getMaxEV(mg_SCrecipower))[1:52,],
     plot(maxEV ~ time, ylim = c(0, 1), type = "l", lwd = 3, yaxs = "i",
...
```{r}
fits <- list("reciprocal" = mg_SCreci,
             "homogeneous" = mg_SChom,
             "power-adjusted" = mg_SCrecipower)
```
...
```{r owas_pit, fig.width = 6, fig.height = 2, echo = -1, results = "hide"}
par(mfrow = c(1,3), mar = c(5,5,1,1), las = 1)
mapply(function(x, main) pit(x, plot = list(main = main)),
       x = owas, main = names(owas))
```

```{r owas_caltest}
sapply(owas, calibrationTest, which = "dss")["p.value",]
sapply(owas, calibrationTest, which = "logs")["p.value",]
```

```{r owas_scores}
owas_scores <- lapply(owas, scores, which = c("dss", "logs"), individual = TRUE, reverse = FALSE)
lapply(owas_scores, apply, 2:3, mean)
```

```{r owas_scores_permutationTest}
set.seed(1)
