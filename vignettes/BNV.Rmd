...
Custom plot of the weekly incidence using **ggplot2**:

```{r BNV_ggplot}
p <- ggplot(tidy.sts(BNV),
            aes(y = observed/(population*sum(pop2011)/100000), x = date)) +
    geom_bar(aes(fill = epochInYear %in% c(52, 1)),
             stat = "identity", show.legend = FALSE) +
    geom_vline(aes(xintercept = as.numeric(date)[4*52] + .5), linetype = 2) +
    scale_fill_manual(values = c("black", "red")) +
    scale_x_date(name = NULL, date_minor_breaks = "months") +
    scale_y_sqrt(name = "weekly incidence [per 100 000 inhabitants]")
p + facet_wrap(~ unit, nrow = 2, ncol = 3)
```
